...
<div x-on:keydown.window.escape="{{ $name }} = false" x-cloak x-show="{{ $name }}"
    ...
    <div class="absolute inset-0 overflow-hidden">
        ...
        <div class="fixed inset-y-0 right-0 max-w-full flex" x-on:click.away="{{ $name }} = false">
            <div x-show="{{ $name }}" x-transition:enter="transform transition ease-in-out duration-500 sm:duration-700"
                x-transition:enter-start="translate-x-full" x-transition:enter-end="translate-x-0"
                x-transition:leave="transform transition ease-in-out duration-500 sm:duration-700"
                x-transition:leave-start="translate-x-0" x-transition:leave-end="translate-x-full"
                {{ $attributes->merge(['class' => 'relative w-screen max-w-lg']) }}>
                <div
                    class="h-full py-4 sm:py-5 px-4 sm:px-6 flex flex-col bg-white border-l border-gray-100 overflow-y-auto">
                    {{ $slot }}
                </div>
            </div>
        </div>
