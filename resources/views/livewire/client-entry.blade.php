<li x-data="{open : false, deleteModal : @entangle('deleteModal'), editModal : @entangle('editModal')}">
        ...
        <div class="flex items-center gap-5">
            ...
            <div>
                <div class="flex items-center gap-x-2">
                    <a href="{{ route('client.show', ['client' => $client]) }}" class="text-base font-semibold text-zinc-900">{{ $client->name }}</a>
                    @if ($client->wasRecentlyCreated)
                        <x-badge class="bg-lime-50 text-lime-700">Recently Added</x-badge>
                    @endif
                </div>
                <div class="text-sm font-medium text-zinc-600">{{ $client->address_line1 }}</div>
            </div>
        </div>
                ...
                <div x-cloak
                    ...
                    <div class="py-1" role="none">
                        <a href="{{ route('client.show', ['client' => 1]) }}"
                            class="text-zinc-700 font-medium hover:bg-zinc-100 hover:text-zinc-900 block px-4 py-2 text-sm"
    ...
    <div x-show="open" x-collapse class="flex flex-col">
        <div class="grid grid-cols-2 py-6 gap-5 bg-white">
            <div class="px-[4.5rem] flex flex-col gap-y-8">
                <div>
                    <div class="mb-1 text-sm text-zinc-600">Name:</div>
                    <div class="text-base font-medium text-zinc-900">{{ $client->name }}</div>
                </div>
                <div>
                    <div class="mb-1 text-sm text-zinc-600">Address:</div>
                    <div class="text-base font-medium text-zinc-900">Antifašističke borbe 37,<br> 11070 Beograd,
                        Srbija</div>
                </div>
                <div>
                    <div class="mb-1 text-sm text-zinc-600">MB (matični broj):</div>
                    <div class="text-base font-medium text-zinc-900">{{ $client->company_identifier }}</div>
                </div>
                <div>
                    <div class="mb-1 text-sm text-zinc-600">PIB:</div>
                    <div class="text-base font-medium text-zinc-900">{{ $client->vat_in }}</div>
                </div>
            </div>
            <div class="px-[4.5rem] flex flex-col gap-y-8">
                <div>
                    <div class="mb-1 text-sm text-zinc-600">Email:</div>
                    <div class="text-base font-medium text-zinc-900">{{ $client->email }}</div>
                </div>
                <div>
                    <div class="mb-1 text-sm text-zinc-600">Tel / Fax:</div>
                    <div class="text-base font-medium text-zinc-900">{{ $client->contact_number ?? 'No contact phone found.' }}</div>
                </div>
                <div>
                    <div class="mb-1 text-sm text-zinc-600">Invoices:</div>
                    <div class="text-base font-medium text-zinc-900">{{ $client->invoices_count }} @choice('invoice|invoices', $client->invoices_count) found</div>
                </div>
                <div>
                    <div class="mb-1 text-sm text-zinc-600">Web:</div>
                    <div class="text-base font-medium text-zinc-900">www.baito.rs</div>
                </div>
            </div>
        </div>
    </div>
